...
func DiskIOMetrics() (L []*model.MetricValue) {
	...
	for _, ds := range dsList {
		...
		L = append(L, CounterValue("disk.read.requests", ds.ReadRequests, device))
		L = append(L, CounterValue("disk.read.merged", ds.ReadMerged, device))
		L = append(L, CounterValue("disk.read.sectors", ds.ReadSectors, device))
		L = append(L, CounterValue("disk.msec.read", ds.MsecRead, device))
		L = append(L, CounterValue("disk.write.requests", ds.WriteRequests, device))
		L = append(L, CounterValue("disk.write.merged", ds.WriteMerged, device))
		L = append(L, CounterValue("disk.write.sectors", ds.WriteSectors, device))
		L = append(L, CounterValue("disk.msec.write", ds.MsecWrite, device))
		L = append(L, CounterValue("disk.ios.in.progress", ds.IosInProgress, device))
		L = append(L, CounterValue("disk.msec.total", ds.MsecTotal, device))
		L = append(L, CounterValue("disk.msec.weighted_total", ds.MsecWeightedTotal, device))
	}
	return
}
...
func IOStatsMetrics() (L []*model.MetricValue) {
	...
	for device := range diskStatsMap {
		...
		L = append(L, GaugeValue("disk.read.bytes", float64(delta_rsec)*512.0, tags))
		L = append(L, GaugeValue("disk.write.bytes", float64(delta_wsec)*512.0, tags))
		L = append(L, GaugeValue("disk.avgrq.sz", avgrq_sz, tags))
		L = append(L, GaugeValue("disk.avgqu.sz", float64(IODelta(device, IOMsecWeightedTotal))/1000.0, tags))
		L = append(L, GaugeValue("disk.await", await, tags))
		L = append(L, GaugeValue("disk.svctm", svctm, tags))
